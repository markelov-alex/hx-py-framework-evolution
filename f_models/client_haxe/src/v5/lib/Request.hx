...
class Method
{
	...
	// (so we use special _method property which is supported on the server)
...
//
//	public static function get(?url:String, ?params:Dynamic,
...
//	}
//
//	public static function post(?url:String, ?params:Dynamic,
//						 ?callback:Dynamic -> Void, ?errorCallback:Dynamic -> Void,
//						 ?headers:Map<String, String>, ?format:String):Void
//	{
//		var request = new Request();
...
//	}
	...
	public var defaultFormat = Format.TEXT;
	public var isAntiCacheEnabled = true;
	...
	public function set_method(value:String):String
	{
		...
		if (method != value)
		{
			...
			// (Note: Flash support only GET and POST methods)
			urlRequest.method = if (value == "POST") value else "GET";
		}
		return value;
	}
	...
	public var params(default, set):Dynamic;
	public function set_params(value:Dynamic):Dynamic
	{
		if (params != value || urlRequest.data == null)
		{
			params = value;
			urlRequest.data = new URLVariables();
			if (value != null)
			{
				var valueMap:Map<String, Dynamic> = Std.downcast(value, StringMap);
				if (valueMap != null)
				{
					for (k => v in valueMap)
					{
						Reflect.setField(urlRequest.data, k, v);
					}
				}
				else
				{
	...
	public function new(?url:String, ?params:Dynamic,
						?callback:Dynamic->Void, ?errorCallback:Dynamic->Void,
	...
	public function send(?method:String, ?url:String, ?params:Dynamic,
						 ?callback:Dynamic -> Void, ?errorCallback:Dynamic -> Void,
						 ?headers:Map<String, String>, ?format:String):Void
	{
		close();
		...
		// Workaround for PUT, PATCH and other methods (should be specially parsed by server)
		if (urlRequest.data._method == null)
		{
			urlRequest.data._method = this.method;
		}
		if (isAntiCacheEnabled)
		{
			urlRequest.data.antiCache = Timer.stamp();
		}
	...
	public function get(?url:String, ?params:Dynamic,
						?callback:Dynamic -> Void, ?errorCallback:Dynamic -> Void,
						?headers:Map<String, String>, ?format:String):Void
	{
	...
	public function post(?url:String, ?params:Dynamic,
						 ?callback:Dynamic -> Void, ?errorCallback:Dynamic -> Void,
						 ?headers:Map<String, String>, ?format:String):Void
	{
