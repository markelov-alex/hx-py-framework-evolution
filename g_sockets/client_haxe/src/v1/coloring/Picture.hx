...
class Picture extends Component
{
	...
	public function new()
	{
		...
		model.applyColorSignal.add(model_applyColorSignalHandler);
		
		// TODO replace 15 code lines with only 1 (property=name by default):
		//public function bind(name:String, model:Model, property:String=null):Void 
	...
	override public function dispose():Void
	{
		...
		if (model != null)
		{
			...
			model.applyColorSignal.remove(model_applyColorSignalHandler);
			model = null;
		}
	}
	...
	private function pictureItem_clickSignalHandler(event:MouseEvent):Void
	{
		...
		model.applyColor(itemIndex, model.color);
	}

	private function model_applyColorSignalHandler(itemIndex:Int, color:Int):Void
	{
