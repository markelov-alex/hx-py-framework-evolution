 ...
 * 
 * Changes:
 *  - get singleton not only by given type, but also try its registered realType.
 */
class IoC
{
	// Settings
	...
	// Use type:Dynamic, for example, to register one Dresser by another Dresset, which 
	// both extend Component, not one another
//	public function register<T>(type:Class<T>, realType:Class<T>):Void
	 ...
	 */
	public function getSingleton<T>(type:Dynamic):T
	{
		if (type == null)
		{
			return null;
		...
		if (instance == null)
		{
			...
			{
				// Get by alias
				instance = getSingleton(realType);
			}
			if (instance == null)
			{
				// Create
			...
			// Save by base type
			singletonByTypeName[typeName] = instance;
			// Save also by current (real) type
