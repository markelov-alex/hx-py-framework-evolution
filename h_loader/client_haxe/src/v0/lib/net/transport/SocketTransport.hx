package v0.lib.net.transport;

import openfl.events.ProgressEvent;
import openfl.utils.ByteArray;
import openfl.utils.Endian;
import v0.lib.util.BytesUtil;
 ...
 * 
 * Changes:
 *  - isBinary -> isOutputBinary, isInputBinary.
 */
class SocketTransport extends BinarySocketTransport
{
	...
	override private function socket_socketDataHandler(event:ProgressEvent):Void
	{
		if (inputBuffer.length > 0)
		{
			...
			if (!isInputBinary)
			{
				Log.debug(' (Prev buffer as utf: ${BytesUtil.toUTF(inputBuffer)})');
			}
		}
		...
		if (!isInputBinary)
		{
			Log.debug(' (Received as utf: ${BytesUtil.toUTF(inputBuffer)})');
		}
		...
		while ((inputBuffer.position:Int) < bufferLength)
		{
			...
			else
			{
				...
				{
					// Read item
					var itemEnd = inputBuffer.position;
					inputBuffer.position = itemStart;
					var data = inputBuffer.readUTFBytes(itemEnd - itemStart);
