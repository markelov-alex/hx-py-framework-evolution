...
class Resizer extends Component
{
	...
	// Don't disable if parent disabled (test on dialogs under other modal dialogs)
 ...
 * 
 * Changes:
 *  - some minor changes (set stage directly, not as sizeSource).
 */
class StageResizer extends Resizer
{
	...
	private function set_stage(value:Stage):Stage
	{
		...
		if (stage != null)
		{
			...
			{
				// Stage size to which all skins were drawn
	...
	override private function assignSkin():Void
	{
		...
		// Before resize() called in set_stage()
		stretchBackground = resolveSkinPath(stretchBackgroundPath);
		//todo apply change on previous versions
		//was 
