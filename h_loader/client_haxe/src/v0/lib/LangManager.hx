...
class LangManager
{
	...
	public function load(assetName:String=null):Void
	{
		...
		if (data != null)
		{
			...
			{
				var value:Dynamic = isYamlMap ? data.get(key) : Reflect.getProperty(data, key);
				// Set up manager
				...
				var keys2 = isYamlMap ? value.keys() : Reflect.fields(value).iterator();
				for (key2 in keys2)
				{
					try
					{
						var value2 = isYamlMap ? value.get(key2) : Reflect.getProperty(value, key2);
						dict[key2] = value2;
					}
					catch (e:Exception)
					{
