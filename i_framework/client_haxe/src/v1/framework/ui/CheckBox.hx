...
class CheckBox extends Button
{
	...
	public function set_isChecked(value:Bool):Bool
	{
		if (isChecked != value)
		{
			...
			if (value)
			{
				//audio.playSound(soundChecked);
				checkedSignal.dispatch(this);
			}
			else
			{
				//audio.playSound(soundUnchecked);
				uncheckedSignal.dispatch(this);
			}
			//audio.playSound(soundToggle);
	...
	override private function interactiveObject_clickHandler(event:MouseEvent):Void
	{
		...
		if (isChecked != prevChecked)
		{
			if (isChecked)
			{
				audio.playSound(soundCheckedClick);
			}
			else
			{
				audio.playSound(soundUncheckedClick);
			}
			audio.playSound(soundToggleClick);
		}
