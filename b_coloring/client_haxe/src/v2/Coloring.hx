...
class Coloring extends Component
{
	...
	override private function assignSkin():Void
	{
		...
		if (mc != null)
		{
			pictureContainer = Std.downcast(mc.getChildByName(pictureContainerName), MovieClip);
	...
	private function refreshPicture():Void
	{
		...
		if (picture != null)
		{
			fitSize(picture, pictureContainer.width, pictureContainer.height);
			picture.x = Math.floor(pictureContainer.x + (pictureContainer.width - picture.width) / 2);
			picture.y = Math.floor(pictureContainer.y + (pictureContainer.height - picture.height) / 2);
			mc.addChild(picture);
			pictureItems = [];
