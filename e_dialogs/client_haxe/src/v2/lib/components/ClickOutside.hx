...
//	}
	...
	private function skin_clickHandler(event:MouseEvent):Void
	{
		...
		isSkinClicked = true;
		//Log.debug('## $this for $parent @Click on skin -> isSkinClicked=true');
		// Remove as it can break normal behavior of other components
	...
	private function stage_clickHandler(event:MouseEvent):Void
	{
		if (isSkinClicked)
		{
			//Log.debug('## $this for $parent @Click on stage (isSkinClicked==true -> Skip)');
			// Clicked inside
			isSkinClicked = false;
			return;
		}
		...
		if (!isEnabled)
		{
			//Log.debug('## $this for $parent @Click on stage (disabled -> Skip)');
			return;
		}
		//Log.debug('## $this for $parent @Click on stage -> dispatch clickOutsidSignal');
